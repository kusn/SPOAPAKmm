...
namespace RSSigGen.RS
{
    //
    // Сводка:
    //     RsSmab_Trace_Power_Sweep_Measurement_Transition_Positive_Duration_Display_Annotation_State
    //     commands group definition
    //     Sub-classes count: 0
    //     Commands count: 1
    //     Total commands count: 1
    public class RsSmab_Trace_Power_Sweep_Measurement_Transition_Positive_Duration_Display_Annotation_State
    {
        private readonly CommandsGroup _grpBase;

        internal RsSmab_Trace_Power_Sweep_Measurement_Transition_Positive_Duration_Display_Annotation_State(Core core, ICmdGroup parent)
        {
        ...
        public void Set(bool state, TraceRepCap trace)
        {
            ...
            _grpBase.IO.Write("TRACe" + repCapCmdValue + ":POWer:SWEep:MEASurement:TRANsition:POSitive:DURation:DISPlay:ANNotation:STATe " + state.ToBooleanString());
        }
        ...
        {
            string repCapCmdValue = _grpBase.GetRepCapCmdValue(trace);
            return _grpBase.IO.QueryBoolean("TRACe" + repCapCmdValue + ":POWer:SWEep:MEASurement:TRANsition:POSitive:DURation:DISPlay:ANNotation:STATe?");
        }
    }
}
